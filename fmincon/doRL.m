function [predictedResp, choiceProb, V] = doRL(alle, p)
    ...
    for ei = 1:nTrials
       ...
       predictedResp(ei) = 2 - ( 1 ./ (1 + exp(-1*beta*(V1-V2))) );  
        ...
        % decay to 0
%        V = (1-d)*V; % decay everything
%        V(mx(moveChoice), my(moveChoice)) = V(mx(moveChoice), my(moveChoice))/(1-d); % undo decay on chosen location
